...
def _detection_layer(inputs, num_classes, anchors, img_size, data_format):
    '''
    在分析本函数代码时，(1) 发现维度不匹配, box_centers的第0维是否是barch_size
    '''
    ...
    # [batch_size, num_anchors*dim, 5+num_classes]
    ...
    # [batch_size, num_anchors*dim, 2]
    box_centers = tf.nn.sigmoid(box_centers)
    # [batch_size, num_anchors*dim, 1]
    ...
    # [batch_size, num_anchors*grid_size[0]*grid_size[1], num_classes]
...
def _upsample(inputs, out_shape, data_format='NCHW'):
    '''对inputs进行上采样
    Args:
        inputs -- the input to be upsampled
        out_shape -- the shape of output
        data_format -- the format of inputs
    '''
    # if tf.image.resize_nearest_neighbor accepts input in format NHWC
    if data_format == 'NCHW':
...
def yolo_v3(inputs, num_classes, is_training=False, data_format='NCHW', reuse=False):
    ...
    Args:
        inputs: a 4-D tensor of size [batch_size, height, width, channels].
...
def detections_boxes(detections):
    ...
    Args:
        detections: outputs of YOLO v3 detector of shape (?, 10647, (5 + num_classes))
...
def _iou(box1, box2):
    ...
    inter_area = (int_x1 - int_x0) * (int_y1 - int_y0)  

    b1_area = (b1_x1 - b1_x0) * (b1_y1 - b1_y0)  # area of box1
    b2_area = (b2_x1 - b2_x0) * (b2_y1 - b2_y0)  # area of box2
...
def non_max_suppression(predictions_with_boxes, confidence_threshold, iou_threshold=0.4):
    ...
    return:
      result -- a dictionary, its element is (box, score)
    """
